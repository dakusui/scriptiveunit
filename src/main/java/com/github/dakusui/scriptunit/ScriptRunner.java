...
public abstract class ScriptRunner extends ParentRunner<Action> {
  ...
  static class GroupingByTestOracle extends ScriptRunner {
    GroupingByTestOracle(Class<?> testClass, String testSuiteDescription, int testOracleId, TestOracle testOracle, List<TestCase> testCases) throws InitializationError {
      super(
          testClass,
          testCases.stream()
              .map(input -> testOracle.createTestAction(testSuiteDescription, testOracleId, input))
              .collect(toList()),
          testOracleId
      );
    }
  ...
  static class GroupingByTestCase extends ScriptRunner {
     ...
     */
    GroupingByTestCase(Class<?> testClass, String testSuiteDescription, int testCaseId, TestCase testCase, List<TestOracle> testOracles)
        throws InitializationError {
      super(
          testClass,
          testOracles.stream()
              .map((TestOracle input) -> input.createTestAction(testSuiteDescription, testCaseId, testCase))
              .collect(toList()),
          testCaseId
      );
    }
