...
import static com.github.dakusui.scriptiveunit.libs.extras.ResponseChecker.*;
import static java.util.Objects.requireNonNull;
import static java.util.stream.Collectors.toSet;

public class SearchEngineLib<REQ extends SearchEngine.Request, RESP extends SearchEngine.Response<DOC>, DOC> {
  ...
  public Value<ResponseChecker<REQ, RESP, DOC, List<DOC>>> nonEmpty() {
    return stage -> new ResponseChecker.Docs<REQ, RESP, DOC>() {
      @Override
  ...
  @Scriptable
  public Value<ResponseChecker.DocsMetric<DOC, REQ, RESP>> dcgBy(Value<Predicate<? super Double>> criterion, Value<Function<DOC, Double>> relevance, Value<Integer> p) {
    return stage -> dcgChecker(relevance.apply(stage), p.apply(stage), criterion.apply(stage));
  }

  @Scriptable
  public Value<ResponseChecker.DocsMetric<DOC, REQ, RESP>> ndcgBy(Value<Predicate<? super Double>> criterion, Value<Function<DOC, Double>> relevance, Value<Function<REQ, Double>> iDcgValue, Value<Integer> p) {
    return stage -> ndcgChecker(relevance.apply(stage), p.apply(stage), iDcgValue.apply(stage).apply(null), criterion.apply(stage));
  }

  @Scriptable
  public Value<ResponseChecker.DocsMetric<DOC, REQ, RESP>> precisionBy(Value<Predicate<? super Double>> criterion, Value<Predicate<DOC>> cond) {
    return stage -> precisionChecker(cond.apply(stage), criterion.apply(stage));
  }

  @Scriptable
  public Value<ResponseChecker.DocsMetric<DOC, REQ, RESP>> precisionByKnownRelevantDocIds(Value<Predicate<? super Double>> criterion, ValueList<String> valueDocIds) {
  ...
  @Scriptable
  public Value<ResponseChecker.DocsMetric<DOC, REQ, RESP>> precisionByKnownIrrelevantDocIds(Value<Predicate<? super Double>> criterion, ValueList<String> valueDocIds) {
  ...
  @Scriptable
  public
  Value<ResponseChecker.DocsMetric<DOC, REQ, RESP>>
  precisionIs(Value<Predicate<DOC>> cond, Value<Predicate<? super Double>> criterion) {
    return stage -> precisionChecker(cond.apply(stage), criterion.apply(stage));
  }


  private RESP service(REQ request) {
    return searchEngine.service(request);
  }
  ...
  private static
  <DOC, REQ extends SearchEngine.Request,  RESP extends SearchEngine.Response<DOC>, T> Value<Boolean>
